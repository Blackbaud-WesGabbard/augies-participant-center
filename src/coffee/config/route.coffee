...
angular.module 'trPcApp'
  .run [
    ...
    'SocialShareService'
    'APP_INFO'
    ($rootScope, $location, $route, $uibModal, $window, AuthService, TeamraiserEventService, TeamraiserRegistrationService, SocialShareService, APP_INFO) ->
      $rootScope.scrollToTop = ->
        $embedRoot = angular.element(document).find '[data-embed-root]'
        if angular.isElement $embedRoot
          $embedRoot[0].scrollIntoView()
      ...
      $rootScope.$on '$routeChangeStart', ($event, next, current) ->
        ...
        else if not $rootScope.socialSettings.janrainInitialized
          $event.preventDefault()
          SocialShareService.initJanrain()
            .then ->
              reloadRoute()
